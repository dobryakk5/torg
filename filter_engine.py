...
def _filter_profile(tender: dict[str, Any], text: str, stage: str) -> FilterScore:
    ...
    # Типовая 1С без Битрикс/web/API/интеграции — не наш профиль: стоп-фактор (→ NO-GO),
    # такие лоты не должны попадать даже в Stage 2. Если в названии есть
    # «битрикс/bitrix/интеграц/обмен данных/api/web/веб/сайт» — это уже профильный лот
    # (сработает _PREFERRED_1C_RE), и он проходит дальше как обычно.
    if _GENERIC_1C_RE.search(title_norm := _normalize(str(tender.get("title") or ""))):
        if not _PREFERRED_1C_RE.search(title_norm):
            return FilterScore(
                1, name, 1,
                ["✗ типовая 1С без web/API/Битрикс-интеграции — не наш профиль"],
                stop_factor=True,
            )
    ...
    # Перекуп = стоп-фактор (→ NO-GO): подтверждено ручными отказами — все
    # «предоставление/продление прав, лицензий, сертификатов, доступа» пользователь
    # отбраковывал кнопкой. Раньше был только кап Ф1, и лоты выживали в CAUTION.
    #
    # Вердикт считаем по НАЗВАНИЮ лота: в полном тексте документов сервисные слова
    # («настройка», «внедрение») встречаются случайно и «спасали» чистый перекуп,
    # а стандартная оговорка «передача неисключительных прав на результат работ»
    # из глубин ТЗ, наоборот, давала ложный перекуп хорошим лотам. Предмет закупки
    # всегда назван в заголовке.
    title_norm = _normalize(str(tender.get("title") or ""))
    resale_basis = title_norm or text[:400]
    resale = _resale_verdict(resale_basis)
    if resale == "resale":
        return FilterScore(
            1, name, 1,
            ["✗ перекуп: лицензии/поставка без работ — не наш профиль"],
            stop_factor=True,
        )
    if resale == "resale_weak":
        return FilterScore(
            1, name, 1,
            ["✗ право использования/лицензия + лишь сопровождение — по сути перекуп"],
            stop_factor=True,
        )

    # «Минимальное сопровождение»: чистое сопровождение/обслуживание/техподдержка
    # без разработки — лёгкий минус (кап 3), НЕ стоп: сопровождение сайта берём.
    if _SUPPORT_ONLY_RE.search(title_norm) and not _DEV_WORDS_RE.search(title_norm):
        score = min(score, 3)
        signals.append("⚠ чистое сопровождение/поддержка без разработки — лёгкий минус")
...
def _filter_scope(tender: dict[str, Any], text: str, stage: str) -> FilterScore:
    ...
    # Работаем только онлайн: любой реальный выезд/работа на объекте — стоп,
    # регион не важен. Шаблонные «доставка по месту нахождения заказчика»
    # витрин ЗМО отфильтрованы контекстом внутри _has_onsite.
    onsite = _has_onsite(_work_scope_text(tender, text))
    if onsite:
        score = 1
        signals = ["✗ требуется выезд/работа на объекте — работаем только онлайн"] + signals

    stop = onsite or bool(place_stops) or len(unlimited) >= 2 or any("неогранич" in x for x in unlimited)
...
# В проект контракта уже вписан конкретный исполнитель: «ООО «Ромашка» …,
# именуемое в дальнейшем «Исполнитель»». Так публикуют закупки у единственного
# поставщика (п. 5 ч. 1 ст. 93) — участвовать в них бессмысленно, контракт
# фактически заключён. Пустой шаблон («___, именуемый в дальнейшем
# «Исполнитель»») под паттерн не попадает: перед «именуем…» нет названия ООО.
_NAMED_EXECUTOR_RE = re.compile(
    r"(?:ООО|АО|ПАО|ИП|общество с ограниченной ответственностью)\s*"
    r"[«\"][^«»\"]{2,80}[»\"]"          # название компании в кавычках
    r"[^«»\n]{0,120}?"                   # (далее — ООО «…»), запятые
    r"именуем\w{0,4}\s+в\s+дальнейшем\s+[«\"]?Исполнитель",
    re.I,
)


def _named_executor_in_contract(text: str) -> bool:
    return bool(_NAMED_EXECUTOR_RE.search(text or ""))
...
def _filter_tailored(tender: dict[str, Any], text: str, stage: str) -> FilterScore:
    ...
    # Жёсткий признак (виден только по документам, т.е. со Stage 2): исполнитель
    # уже вписан в проект контракта — закупка под конкретную компанию.
    if stage != "stage1" and _named_executor_in_contract(text):
        return FilterScore(
            6, name, 1,
            ["⛔ в проект контракта уже вписан конкретный исполнитель — закупка под него"],
            stop_factor=True,
        )
...
# Перекуп ловим регулярками, а не подстроками: у ЕИС/ЕАТ формулировки идут в любом
# падеже и со вставками в скобках — «предоставлению (продлению) лицензий»
# подстрока "предоставление лицензи" не находила, и перекуп проходил как профильный.
_RESALE_LICENSE_RE = re.compile(
    r"прав[оаы]?\s+(?:на\s+)?использовани"          # право/права (на) использование
    r"|неисключительн"                                # неисключительные права
    r"|(?:поставк|предоставлен|продлен|передач|приобретен|закупк)\w*"
...
_RESALE_GOODS_RE = re.compile(
    r"(?:поставк|приобретен|закупк)\w*"
    r"\s*(?:\([^)]{0,40}\)\s*)?"
    r"(?:\w+\s+){0,2}?"
    # «программн» здесь = поставка ПО как товара; лот с внедрением/настройкой
    # спасают сильные сервисные маркеры (см. _resale_verdict).
    r"(?:оборудовани|сервер|товар|литератур|мебел|канцеляр|программн)",
    re.I,
)
# Совместимость: часть кода/тестов ожидает список подстрок.
_RESALE_MARKERS = [
...
def _has_resale_marker(text: str) -> bool:
    return bool(_RESALE_LICENSE_RE.search(text) or _RESALE_GOODS_RE.search(text))
# Сервисные маркеры разделены по весу: не всякая «услуга» рядом с лицензией
# означает нашу работу. «Право использования ПО + сопровождение» — это перекуп
# с формальной поддержкой от вендора (её оказывает правообладатель, не мы), а вот
# «право использования + внедрение/доработка/интеграция» — уже реальные работы.
_SERVICE_STRONG_MARKERS = [
    "настройк", "внедрен", "доработк", "разработк", "интеграц",
    "модернизац", "администрирован", "миграц", "адаптац",
]
_SERVICE_WEAK_MARKERS = [
    "сопровожден", "обслуживани", "техническая поддержк", "техническое сопровожд",
    "консультац",
]
# Совместимость: часть кода/тестов ожидает единый список.
_SERVICE_MARKERS = _SERVICE_STRONG_MARKERS + _SERVICE_WEAK_MARKERS
...
_OFFPROFILE_MARKERS = [
    ...
    # Справочно-правовые системы: обслуживать экземпляры может только дилер
    # правообладателя — де-факто «под своего», нам не интересно.
    "консультантплюс",
    "консультант плюс",
    "гарант-",
    "спс гарант",
...
# Классы, выведенные из ручных отказов пользователя (кнопка «отказ», 76 лотов):
# каждый пункт подтверждён хотя бы одним реально отбракованным лотом.
# Регулярки, а не подстроки — падежи («услуг связи», «доступа в …») ловятся.
_OFFPROFILE_RE_LIST: list[tuple[re.Pattern, str]] = [
    (re.compile(r"услуг\w*\s+связи", re.I), "услуги связи"),
    (re.compile(r"(?:доступ\w*|подключени\w*)\s+(?:в|к)\s+(?:сет\w+\s+)?"
                r"(?:интернет|информационно-телекоммуникацион)", re.I), "доступ в интернет"),
    (re.compile(r"контентн\w*\s+фильтрац", re.I), "контентная фильтрация"),
    (re.compile(r"видеонаблюден", re.I), "видеонаблюдение"),
    (re.compile(r"антивирусн", re.I), "антивирусы (перекуп лицензий)"),
    (re.compile(r"фстэк|скзи|криптографическ", re.I), "сертифицированные средства защиты (ФСТЭК/СКЗИ)"),
    (re.compile(r"анализ\w*\s+защищ[её]нност", re.I), "анализ защищённости"),
    (re.compile(r"бесперебойного\s+питани", re.I), "ИБП/электропитание"),
    (re.compile(r"кондиционер|систем\w*\s+кондиционировани", re.I), "кондиционеры/климатика"),
    (re.compile(r"структурированн\w*\s+кабельн", re.I), "монтаж СКС"),
    (re.compile(r"полиграфическ", re.I), "полиграфия"),
    (re.compile(r"баннер", re.I), "баннеры/наружка"),
    (re.compile(r"размещени\w*\s+информационн\w*\s+материал", re.I), "размещение публикаций/СМИ"),
    (re.compile(r"проведени\w*\s+обучени|образовательн\w*\s+услуг", re.I), "обучение/образовательные услуги"),
    (re.compile(r"\bниокр\b|опытно-конструкторск", re.I), "НИОКР"),
    (re.compile(r"продаж\w*\s+неликвид|объявление\s+о\s+продаже", re.I), "торги по продаже имущества"),
    (re.compile(r"перевод\w*\s+денежных\s+средств", re.I), "финансовые услуги"),
    (re.compile(r"котельн", re.I), "котельные/теплоснабжение"),
    (re.compile(r"трансформаторн|электроподстанци", re.I), "электрохозяйство"),
    (re.compile(r"подготовк\w*\s+и\s+размещени\w*\s+(?:\w+\s+){0,3}?информаци"
                r"|освещени\w*\s+деятельност", re.I), "публикации/освещение в СМИ"),
    (re.compile(r"аттестац\w*\s+(?:\w+\s+){0,3}?(?:информационн|испдн|объект\w*\s+информатизац)", re.I),
     "аттестация ИС/ИСПДн (нужна лицензия ФСТЭК)"),
]

# «Минимальное сопровождение»: сопровождение/поддержка без слов разработки —
# лёгкий минус в Ф1 (кап 3), проверяется по НАЗВАНИЮ лота (в полном тексте
...
def _resale_verdict(text: str) -> str:
    """Классифицирует лот по признаку перекупа:

    "resale"      — поставка/лицензии без каких-либо работ (жёсткий перекуп);
    "resale_weak" — лицензии/поставка + только формальные услуги (сопровождение,
                    техподдержка): такую поддержку оказывает правообладатель, для
                    нас это тот же перекуп, лишь слегка прикрытый;
    ""            — либо перекупа нет, либо рядом есть реальные работы
                    (внедрение/доработка/интеграция/настройка).
    """
    if not _has_resale_marker(text):
        return ""
    if any(m in text for m in _SERVICE_STRONG_MARKERS):
        return ""
    if any(m in text for m in _SERVICE_WEAK_MARKERS):
        return "resale_weak"
    return "resale"
...
def _is_resale_without_service(text: str) -> bool:
    ...
    return _resale_verdict(text) == "resale"


def _offprofile_markers(text: str) -> list[str]:
    found = [marker for marker in _OFFPROFILE_MARKERS if marker in text]
    for pattern, label in _OFFPROFILE_RE_LIST:
...
# Подстроки выше не ловят падежи («выездом на объект», «выезда специалиста»),
# поэтому наличие выезда определяем регуляркой. (?<!без ) отсекает «без выезда».
_ONSITE_RE = re.compile(
    r"(?<!без )выезд\w*\s+(?:на\s+объект|специалист|на\s+территори|к\s+заказчик|на\s+мест)"
    r"|с\s+выезд\w*"
...
# Шаблоны витрин ЗМО пишут «доставка … по месту нахождения заказчика» в КАЖДОМ
# лоте — это адрес ПОСТАВКИ, а не требование работать на объекте. Совпадение
# в таком контексте не считается выездом (реальный кейс: лицензия РАМЗЭС
# получала флаг «выезд» из шаблонного блока «Условия поставки» ЕАТ).
_DELIVERY_CTX_RE = re.compile(r"доставк|поставк|график\s+поставки|место\s+доставки", re.I)


def _has_onsite(text: str) -> bool:
    for m in _ONSITE_RE.finditer(text):
        ctx = text[max(0, m.start() - 80):m.end() + 80]
        if not _DELIVERY_CTX_RE.search(ctx):
            return True
    return False
...
def detect_flags(text: str) -> dict:
    ...
    onsite = (not empty) and _has_onsite(t)
