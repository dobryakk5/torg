...
def _filter_profile(tender: dict[str, Any], text: str, stage: str) -> FilterScore:
    ...
    resale = _resale_verdict(text)
    if resale == "resale":
        score = min(score, 2)
        signals.append("⚠ перекуп/поставка без работ по настройке — не наш профиль")
    elif resale == "resale_weak":
        # Лицензия + сопровождение/техподдержка: работ по сути нет, поддержку даёт
        # правообладатель. Штрафуем мягче жёсткого перекупа, но лот всё равно не наш.
        score = min(score, 3)
        signals.append("⚠ право использования/лицензия + только сопровождение — по сути перекуп")
...
_RESALE_GOODS_RE = re.compile(
    ...
    r"\s*(?:\([^)]{0,40}\)\s*)?"
    r"(?:\w+\s+){0,2}?"
    r"(?:оборудовани|сервер|товар|литератур|мебел|канцеляр)",
    re.I,
)
...
def _non_moscow_work_place_risks(tender: dict[str, Any], text: str) -> tuple[list[str], list[str]]:
    ...
    # Любой выезд в немосковский лот = командировка, а командировки не берём.
    # Для московских лотов эта функция вообще не вызывается (см. выход выше),
    # поэтому выезд «к себе в город» стоп-фактором не становится.
    if _has_onsite(scope_text):
        stops.append("требуется выезд/работа на объекте вне Москвы (командировка)")
...
# поэтому наличие выезда определяем регуляркой.
_ONSITE_RE = re.compile(
    r"выезд\w*\s+(?:на\s+объект|специалист|на\s+территори|к\s+заказчик|на\s+мест)"
    r"|с\s+выезд\w*"
...
def _has_onsite(text: str) -> bool:
    return bool(_ONSITE_RE.search(text)) or any(m in text for m in _ONSITE_MARKERS)
