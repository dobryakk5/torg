...
def process(tenders, phrase, *, save_details, refresh_details) -> tuple[int, int, int, int]:
    """Скоринг + upsert + детали (только для не-NO-GO 44-ФЗ).
    ...
    for tender in tenders:
        ...
        if save_details and tender.get("law_type") == "44-ФЗ":
...
def main() -> None:
    ...
    ap.add_argument("--include-today", action="store_true", help="Включить сегодняшний день (для ежедневного прогона вперёд)")
    args = ap.parse_args()
    ...
    start_offset = 0 if args.include_today else 1         # 0 = включая сегодня
    for offset in range(start_offset, args.days + 1):     # 0=сегодня, 1=вчера, 2=позавчера…
        day = today - timedelta(days=offset)
        date_iso = day.isoformat()                        # YYYY-MM-DD (в файл прогресса)
        date_ru  = day.strftime("%d.%m.%Y")               # дд.мм.гггг (для ЕИС)
        logger.info("─" * 64)
        logger.info("📅 ДЕНЬ %s (%s)", date_iso, "сегодня" if offset == 0 else f"вчера-{offset - 1}")
