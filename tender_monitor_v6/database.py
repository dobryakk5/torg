...
SCHEMA_VERSION = "v5"   # bump при добавлении новых таблиц
...
def init_db() -> None:
    ...
    with _conn() as conn:
        ...
        for ddl in (
            ...
            DDL_KB, DDL_INDEXES, DDL_TRIGGER,
        ):
            cur.execute(ddl)
        cur.execute(
...
def _kb_list(table: str, where: str = "", params: tuple = ()) -> list[dict]:
    with _conn() as conn:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute(f"SELECT * FROM {table} {where} ORDER BY id DESC", params)
        return [_to_jsonable(dict(r)) for r in cur.fetchall()]


def _kb_insert(table: str, data: dict) -> int:
    cols   = ", ".join(data.keys())
    vals   = ", ".join(f"%({k})s" for k in data.keys())
    with _conn() as conn:
        cur = conn.cursor()
        cur.execute(f"INSERT INTO {table} ({cols}) VALUES ({vals}) RETURNING id", data)
        return cur.fetchone()[0]


def _kb_update(table: str, row_id: int, data: dict) -> None:
    sets = ", ".join(f"{k} = %({k})s" for k in data.keys())
    data["_id"] = row_id
    with _conn() as conn:
        cur = conn.cursor()
        cur.execute(f"UPDATE {table} SET {sets} WHERE id = %(_id)s", data)
...
def kb_contracts_list() -> list[dict]:
    return _kb_list("kb_contracts")


def kb_contracts_by_category(category: str, limit: int = 5) -> list[dict]:
    return _kb_list("kb_contracts", "WHERE category = %s LIMIT %s", (category, limit))


def kb_contract_save(data: dict) -> int:
...
def kb_competencies_list() -> list[dict]:
    return _kb_list("kb_competencies", "ORDER BY category, level DESC, tech")


def kb_competencies_for_category(category: str) -> list[dict]:
    """Возвращает компетенции, релевантные категории тендера."""
    # Простой матчинг: если категория встречается в tech или notes
    with _conn() as conn:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("""
            SELECT * FROM kb_competencies
            WHERE active IS DISTINCT FROM FALSE
            ORDER BY level DESC, tech
        """)
        return [_to_jsonable(dict(r)) for r in cur.fetchall()]


def kb_competency_save(data: dict) -> int:
    row_id = data.pop("id", None)
    if row_id:
...
def kb_equipment_list() -> list[dict]:
    return _kb_list("kb_equipment", "ORDER BY category, name")


def kb_equipment_find_analogs(text: str) -> list[dict]:
    """Ищет оборудование, которое является аналогом для брендов/моделей из текста ТЗ."""
    text_lower = text.lower()
    with _conn() as conn:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("""
            SELECT * FROM kb_equipment
            WHERE analog_for && ARRAY(
                SELECT unnest(analog_for)
                FROM kb_equipment
            )
        """)
        rows = cur.fetchall()
    # Фильтруем: аналог для бренда, который упомянут в тексте
    result = []
    for r in rows:
        d = _to_jsonable(dict(r))
        brands = d.get("analog_for") or []
        if isinstance(brands, str):
            import re
            brands = re.findall(r"[\w\-]+", brands)
        if any(b.lower() in text_lower for b in brands):
            result.append(d)
    return result[:5]


def kb_equipment_save(data: dict) -> int:
    if isinstance(data.get("specs"), dict):
        data["specs"] = json.dumps(data["specs"], ensure_ascii=False)
    if isinstance(data.get("analog_for"), list):
        data["analog_for"] = data["analog_for"]   # psycopg2 передаёт list как array
    row_id = data.pop("id", None)
    if row_id:
...
def kb_risk_rules_list() -> list[dict]:
    return _kb_list("kb_risk_rules", "ORDER BY rule_type, weight")


def kb_risk_rules_active() -> list[dict]:
    return _kb_list("kb_risk_rules", "WHERE active = TRUE ORDER BY weight")


def kb_risk_rule_save(data: dict) -> int:
...
def kb_templates_list() -> list[dict]:
    return _kb_list("kb_templates", "ORDER BY category, title")


def kb_template_save(data: dict) -> int:
...
def kb_template_use(row_id: int) -> Optional[dict]:
    """Возвращает шаблон и увеличивает счётчик использований."""
    with _conn() as conn:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute(
...
def kb_stats() -> dict[str, int]:
    try:
        with _conn() as conn:
            cur = conn.cursor()
            result = {}
            for tbl in ("kb_contracts","kb_competencies","kb_equipment",
                        "kb_risk_rules","kb_templates"):
                cur.execute(f"SELECT COUNT(*) FROM {tbl}")
                result[tbl] = cur.fetchone()[0]
        return result
    except Exception:
        return {t: 0 for t in ("kb_contracts","kb_competencies","kb_equipment",
                                "kb_risk_rules","kb_templates")}
