...
# ══════════════════════════════════════════════════════════════════════════════
# KNOWLEDGE BASE — страница + REST API
# ══════════════════════════════════════════════════════════════════════════════
...
# Матчинг компетенций для конкретного тендера
@app.get("/api/kb/match/{purchase_number}")
async def api_kb_match(purchase_number: str):
    from knowledge_base import match_competencies, build_llm_context
    tender = db.get_tender(purchase_number)
    if not tender:
        raise HTTPException(404, "Тендер не найден")
    tz = tender.get("document_text") or tender.get("primary_text") or ""
