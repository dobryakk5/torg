...
# Окно близости по умолчанию (сколько ЛИШНИХ слов допускается между словами фразы).
# Замерено на реальной выдаче ЕИС: заголовки многословные — «доработке
# информационного портала (веб-сайта)» разносит «доработк*» и «сайт*» на 2 слова,
# а рваные слова из HTML ЕИС («модернизац ии», «интернет- сайт ов») добавляют ещё.
# Окно 1 принимало 22 из 62 профильных «сайтовых» лотов, окно 3 — 41, при этом
# мусор не вырос (14 из 799 в обоих случаях). Дальше 3 растёт только шум.
_DEFAULT_WINDOW = 3
...
# Перекуп готового ПО как ПРЕДМЕТ закупки — жёсткое исключение.
# ВАЖНО (проверено на реальной выдаче ЕИС): формулировка «приобретение лицензий»
# в ЕИС не встречается — пишут «предоставление/передача/продление». Фразы ниже
# подобраны так, чтобы НЕ задевать настоящую разработку: в договорах на разработку
# передача неисключительных прав на результат работ — норма, и она же попадает
# в primary_text как договорная болтовня. Поэтому глушим только конструкции,
# где перекуп — подлежащее («услуги ПО ПЕРЕДАЧЕ неисключительных прав»).
_RESALE_LICENSE_HARD = [
    _mh("(предоставлен* неисключительн*)~2", note="предоставление неисключительных прав = перекуп"),
    _mh("(предоставлен* лиценз*)~3", note="предоставление лицензий"),
    _mh("(продлен* лиценз*)~3", note="продление лицензий"),
    _mh("(предоставлен* прав* использован*)~3", note="предоставление права использования"),
    _mh("(приобретен* прав* использован*)~3", note="приобретение права использования"),
    _mh("(продлен* прав* пользован*)~3", note="продление права пользования"),
    _mh("(прав* использовать)~2", note="право использовать ПО"),
    _mh("(услуг* по передач* неисключительн*)~2",
        note="перекуп как предмет закупки; голое «передача неисключительных прав» "
             "НЕ глушим — в разработке это передача прав на результат работ"),
    _mh("(лиценз* на программн* обеспечен*)~2", note="предмет закупки — сама лицензия на ПО"),
]
...
DEFAULT_PROFILES: list[dict[str, Any]] = [
    {
        ...
        "phrases": [
            ...
            *_RESALE_LICENSE_HARD,
            ...
            # Сплошной минус «сопровожден*» здесь НЕ ставим: реальные лоты вида
            # «сопровождение, техподдержка И ДОРАБОТКА портала» — это наша работа,
            # а штраф топил их ниже порога. Чистое сопровождение и так не пройдёт:
            # плюс-фраз на него в этом профиле нет (см. профиль «Сопровождение»).
